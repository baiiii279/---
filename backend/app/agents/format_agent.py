...
1. **论文题目**：确保论文题目以一级标题（#）居中

2. **摘要与关键词**：按格式规则处理
   - 摘要标题使用 `[摘要]` 标记，标题后空一格接内容
   - 关键词标题使用 `[关键词]` 标记，标题后空一格接关键词，词间空两格
   - 摘要与关键词之间空一行

3. **标题层级与对齐**：严格规范所有标题
   - 一级标题（#，章标题如"引言"、"结论"等）：**居中**
   - 二级标题（##，节标题）：**左对齐**
   - 三级标题（###）：**左对齐**
   - 所有标题前后各空一行
...
5. **章节分隔**：
   - **不要添加任何分页标记**。所有章节连续书写，章与章之间仅用一个空行分隔

6. **参考文献格式化**：
   - 按照 GB/T 7714 标准重新编排参考文献格式
   - 确保参考文献不少于10条
   - 参考文献列表前添加 `<!-- format: ref-list -->` 标记
   - 每条文献左顶格，以 [N] 编号，以句号结束

7. **添加格式标记**：在关键位置添加排版指令标记
   - 正文段落前: `<!-- format: body-text -->`
   - 摘要内容前: `<!-- format: abstract -->`
   - 列表前: `<!-- format: list -->`
   - 表格前: `<!-- format: table -->`
