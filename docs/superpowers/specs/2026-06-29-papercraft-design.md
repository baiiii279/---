...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/user/references | 获取我的文献库/收藏列表 |
| POST | /api/user/references | 新增文献/收藏 |
| DELETE | /api/user/references/{id} | 删除文献/收藏 |

> 说明：`user_references` 表直接承载课设"个人页面—我的收藏"需求。用户保存的文献既是个人收藏，又可通过 paper_references 关联到论文作为引用来源，一举两得。
...
| POST | /api/papers/{id}/agent/feedback | 提交审阅反馈（task_id + action + comment + edited_content） |

### 管理员模块
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/admin/users | 用户列表（分页） |
| GET | /api/admin/papers | 全部论文（分页、状态筛选） |
| DELETE | /api/admin/papers/{id} | 强制删除论文 |
...
| 个人中心 | /profile | 头像、修改密码、我的论文列表、我的收藏(文献库入口) |
| 文献库 | /references | 用户个人文献管理（CRUD）= 课设"我的收藏"功能 |
...
```json
{
  ...
  "comment": "修改意见...",
  "edited_content": "（用户修改后的文本，action=edit 时必填）"
}
```

- `action=approve`：编排器进入下一步
- `action=reject`：编排器将 `comment` 注入重跑 prompt，当前 Agent 重新执行
- `action=edit`：编排器用 `edited_content` 替换 Agent 产出，进入下一步
| 润色完成 | 确认/驳回 | 驳回 → 重新润色；确认 → 启动引用检查 |
| 引用检查完成 | 确认/驳回 | 驳回 → 触发修正；确认 → 论文完成 |

## 实时推送 (SSE)

`GET /api/papers/{id}/agent/events` 返回 SSE 流，需 JWT 认证，仅论文所有者可订阅。事件格式：
...
- **版本对比**：润色 Agent 输出修改前后对比，前端用 diff 视图展示润色前后的具体改动
- **导出功能**：论文完成后支持导出为 Word/PDF 格式
- **多语言支持**：支持英文论文写作，Agent 切换相应学术写作风格
- **引用自动补全**：引用检查 Agent 发现引用不足时，自动搜索补充相关文献
